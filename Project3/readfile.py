...
def read_c(filename):
    ...
    lambda_dim = 0
    n_ = np.zeros(n_length)
    variance_ = np.zeros((n_length, 10))
    variance_dim = 0
    ...
    for line in infile:
        if line[0] == '-':
            print(res_)
            plt.subplot(2,1,1)
            plt.plot(n_[:-1], res_[:-1] , label="lambda = %2.3f" %lambd_[lambda_dim])
            plt.legend()
            ...
            lambd_[i] = float(lambd)
            n_[i] = int(n)
            variance_[i, variance_dim] = variance



#obs! plott varians for kun lambda = 1.9, og dropp de andre
#gjør dette for legendre og enkel monte carlo
    ...
    plt.subplot(2,1,2)
    plt.plot(n_[:-1], variance_[:-1, 2], label = "lambda = 1.9")

    #plt.show()
    print(np.shape(variance_))
    plt.ylabel("variance")
